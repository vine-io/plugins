...
func (m Migrator) HasIndex(value interface{}, name string) bool {
	...
	m.RunWithValue(value, func(stmt *dao.Statement) error {
		...
		return m.DB.Raw(
			"SELECT count(*) FROM pg_indexes WHERE tablename = ? AND schemaname = ?", stmt.Table, name, m.CurrentSchema(stmt),
		).Row().Scan(&count)
	})
...
func (m Migrator) CreateTable(values ...interface{}) (err error) {
	...
	for _, value := range m.ReorderModels(values, false) {
		if err = m.RunWithValue(value, func(stmt *dao.Statement) error {
			for _, field := range stmt.Schema.FieldsByDBName {
				if field.Comment != "" {
					if err := m.DB.Exec(
						"COMMENT ON COLUMN ?.? IS ?",
						m.CurrentTable(stmt), clause.Column{Name: field.DBName}, dao.Expr(m.Migrator.Dialect.Explain("$1", field.Comment)),
					).Error; err != nil {
						return err
					}
				}
